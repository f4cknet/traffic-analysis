...
**关键设计**：用 `tcp.stream` 关联 request/response，**只保留 2xx/3xx 响应**作为"真找到"的后台，4xx/5xx 是攻击者的目录扫描噪声（被过滤）。

**`rules/login_paths.yaml`**：29 条规则，5 类别（admin/CMS/db/framework/app）。

**`script/`**：
- `matcher.py` — `match_login_path(rec, rules_data)` uri_path 子串匹配，yaml 顺序优先（精确规则排前避免 overlap）
- `aggregator.py` — `aggregate_login_paths(http_data, rules)` 按 path 聚合（过滤 2xx/3xx）+ `build_attacker_profiles(http_data, rules)` 按 IP 聚合（过滤 2xx/3xx）
- `report.py` — `print_summary(...)` 控制台高可疑登录后台摘要（含完整 URL + 状态码）
- `__init__.py` — 公开 API（re-export）

**`test/`**：31 个 pytest，0.04s。
