...
**`tools/requirements.txt`**（运行时）：
```
PyYAML>=6.0           # 规则库加载（必需）
scapy>=2.5.0          # pcap 解析（可选, --backend scapy 才需要）
```

**`tools/requirements-dev.txt`**（开发时，含 pytest）：
```
-r requirements.txt
pytest>=7.0           # 单测框架
```

**`tools/src/extend-tools/tshark/`**（v0.2.0+ 必需）：
- tshark 4.6.6 二进制子集（110MB / 50 文件）
- 仅 Windows x64；跨平台需单独处理

`pip install -r tools/requirements.txt` 一条命令搞定，跨平台一致。
