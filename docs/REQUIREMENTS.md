...
**v0.2.0 范围**（MVP v3 迁移）：
- [ ] 解析 pcap/pcapng，提取所有 HTTP 请求
- [ ] 按字段分类匹配：UA / header / URI payload
- [ ] 输出攻击者 IP 排行 + 评分
- [ ] 输出攻击时间线
- [ ] 输出 payload 样例
- [ ] 输出 Markdown 格式报告
...
- [ ] 一键导出 HTTP 对象到目录
- [ ] webshell 上传时间轴分析（POST multipart 检测）
- [ ] webshell 内容分析（base64 解码、混淆识别）
- [ ] 多 pcap 批量分析

### 3.2 性能需求

- 170MB pcap（含 7 万 HTTP 请求）全量分析 < 30 秒
- 内存占用 < 2GB
- 报告生成 < 5 秒
...
- Python 3.10+
- tshark 4.x（`D:\hacktools\misc\Wireshark\tshark.exe` 或 PATH 中）
- 不依赖 WSL
