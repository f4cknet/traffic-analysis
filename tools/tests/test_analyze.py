...
from analyzer_core import analyze, classify_uri, is_normal_browser_ua
...
def test_analyze_first_seen_recorded(rules, mixed_records):
    """首次时间应记录 (sqlmap fixture ts=1700000001)"""
    stats = analyze(mixed_records, rules)
    assert "sqlmap" in stats["scanner_first_seen"]
    assert stats["scanner_first_seen"]["sqlmap"] == 1700000001.0


def test_analyze_samples_capped(rules, mixed_records):
    """每个扫描器的样例最多 6 条"""
    stats = analyze(mixed_records, rules)
    for sid, samples in stats["scanner_sample"].items():
...
# ============== is_normal_browser_ua ==============

def test_is_normal_browser_ua_chrome():
    assert is_normal_browser_ua("Mozilla/5.0 ... Chrome/120.0")

def test_is_normal_browser_ua_firefox():
    assert is_normal_browser_ua("Mozilla/5.0 ... Firefox/115.0")

def test_is_normal_browser_ua_sqlmap():
    assert not is_normal_browser_ua("sqlmap/1.2.3.50")

def test_is_normal_browser_ua_empty():
    assert not is_normal_browser_ua("")

def test_is_normal_browser_ua_awvs_disguised():
    """AWVS 默认 UA 伪装成 Chrome 41 — 应被判为正常浏览器 UA (这是已知混淆点)"""
    # 这暴露了 limitations: 单看 UA 不能识别 AWVS, 必须看 header
    assert is_normal_browser_ua("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/41.0.2228.0")